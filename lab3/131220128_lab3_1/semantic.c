...
int lookup_stable(char *text, enum KType *x) {
    ...
    while (tmp != NULL) {
        if (strcmp(text, tmp->name) == 0) {
            //printf("in lookupstable %s\n", text);
            //printf("in lookupstable %d\n", tmp->var_no);
            *x = tmp->symType;
            return tmp->var_no;
        }
        tmp = tmp->nextSym;
...
struct SymbolMsg *getVar_str(char *text) {
    unsigned int hashval = hash_pjw(text);
    struct SymbolMsg *tmp = symbolTable[hashval];
    while (tmp != NULL) {
        if (strcmp(text, tmp->name) == 0) {
            tmp->isParam = 1;
            //printf("in getvarstr %s  %d\n",tmp->name, tmp->isParam);
            return tmp;
        }
        tmp = tmp->nextSym;
    }
